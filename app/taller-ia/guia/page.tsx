// Guía del taller (taller.rossi-ia.com/guia): el método, los conceptos y las
// herramientas, para leer en el celular, descargar en PDF y compartir por
// WhatsApp. Contenido en lib/taller-guia.ts (la misma fuente que el PDF).
...
export default function GuiaTallerPage() {
  return (
    <main className="bg-grid min-h-dvh">
      <div className="mx-auto max-w-4xl space-y-14 px-5 py-10 sm:px-8 sm:py-14">
        ...
        <header className="space-y-6">
          <Logos />
          <div>
            <p className="font-mono text-sm uppercase tracking-[0.3em] text-violet">Guía del taller</p>
        ...
        {/* El método: las 8 etapas */}
        <section>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">El método: 8 etapas</h2>
          <p className="mt-2 text-lg text-muted">El recorrido del taller sirve como método para cualquier conflicto.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-4">
            {GUIA_METODO.map((m, i) => (
              <div key={m.paso} className="glass rounded-2xl p-5">
                <p className="font-mono text-3xl font-bold text-teal">{i}</p>
        ...
        <section>
          ...
          <p className="mt-4 text-base italic text-muted">
            Usado como prompt de sistema (el P0 del taller), el COTIO se escribe una vez y ordena todas las consultas del caso.
          </p>
        </section>

        {/* El caso */}
        <section className="glass rounded-3xl p-6 sm:p-8">
          <p className="font-mono text-sm uppercase tracking-[0.2em] text-violet">{GUIA_CASO.referencia}</p>
          <h2 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">{GUIA_CASO.titulo}</h2>
          <ul className="mt-5 space-y-2 text-lg leading-relaxed">
            {GUIA_CASO.puntos.map((p) => (
              <li key={p}>• {p}</li>
            ))}
          </ul>
          <p className="mt-6 text-base font-semibold text-teal">Lo que cada participante se lleva</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {GUIA_CASO.productos.map((p) => (
