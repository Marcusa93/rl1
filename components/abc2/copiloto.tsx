...
import { buildAbc2CopilotoSystem, type Perfil } from "@/lib/abc2";
import { streamGenerate } from "@/components/use-stream";
import { Spinner } from "@/components/ui";
import { cn } from "@/lib/utils";

const SALUDO = "¡Hola de nuevo! Soy tu copiloto. Cualquier duda en cualquier paso, preguntame.";
const ATAJOS = ["¿Qué hago en este paso?", "No sé qué poner", "Dame un ejemplo de mi rubro"];
...
export function Copiloto({
  ...
  slug: string;
  ctx: { modulo: number; perfil: Perfil | null };
  onUse?: () => void;
}) {
  ...
  async function send(text: string) {
    ...
    try {
      await streamGenerate(
        ...
        { system: buildAbc2CopilotoSystem(ctxRef.current), temperature: 0.4, maxTokens: 600 },
      );
    } catch (e) {
        ...
        <div className="fixed bottom-20 right-5 z-30 flex max-h-[70vh] w-[min(92vw,22rem)] flex-col overflow-hidden rounded-2xl border border-line bg-ink/95 shadow-2xl backdrop-blur rise">
          <div className="flex items-center gap-2 border-b border-line/60 bg-panel/60 px-4 py-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[11px] text-faint">Te ayuda a pensar · no hace la tarea por vos</p>
            </div>
          </div>
