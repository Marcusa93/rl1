...
import { buildAbcCopilotoSystem, type AbcCtx } from "@/lib/abc";
import { streamGenerate } from "@/components/use-stream";
import { Spinner } from "@/components/ui";
import { cn } from "@/lib/utils";

const SALUDO =
  "¡Hola! Soy tu copiloto. Estoy acá para ayudarte en cada paso, con calma. Si algo no se entiende o no sabés qué poner, preguntame nomás.";

const ATAJOS = ["¿Qué hago en este paso?", "No sé qué escribir", "¿Para qué sirve esto?"];

export function Copiloto({ slug, ctx, onUse }: { slug: string; ctx: AbcCtx; onUse?: () => void }) {
  ...
  async function send(text: string) {
    ...
    try {
      await streamGenerate(
        ...
        { system: buildAbcCopilotoSystem(ctxRef.current), temperature: 0.4, maxTokens: 600 },
      );
    } catch (e) {
    ...
    <>
      <button
        ...
        className={cn(
          ...
          open ? "border border-line bg-panel text-muted" : "bg-gradient-to-r from-teal via-cyan to-violet text-ink",
        )}
        aria-label="Abrir el copiloto"
      >
        <span className="text-lg">{open ? "✕" : "💬"}</span>
        {!open && <span>Copiloto</span>}
      </button>
        ...
        <div className="fixed bottom-20 right-5 z-30 flex max-h-[70vh] w-[min(92vw,22rem)] flex-col overflow-hidden rounded-2xl border border-line bg-ink/95 shadow-2xl backdrop-blur rise">
          <div className="flex items-center gap-2 border-b border-line/60 bg-panel/60 px-4 py-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-foreground">Tu copiloto</p>
              <p className="truncate text-[11px] text-faint">Te ayuda paso a paso · no hace la tarea por vos</p>
            </div>
          </div>
              ...
              <div key={i} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    ...
                    m.role === "user" ? "bg-gradient-to-r from-teal to-cyan text-ink" : "border border-line bg-panel/60",
                  )}
                >
